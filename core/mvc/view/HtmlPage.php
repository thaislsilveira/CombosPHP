...
//..se houver array de objetos, então...
        if ($arrayObj) {
            echo "<h2 class='text-center'>LISTA</h2>";
            echo "<hr>";
            echo "<div class='container'>";
            echo "<table class='table table-action'>";
            echo "<thead>";
            echo "<tr>";
            //..cria o cabeçalho percorrendo o array header e criando as células.
            foreach ($header as $field) {
                echo "<th class='t-small'>$field</th>";
            }           
            echo "<th class='t-small' >Editar/Excluir</th>";
             echo "</thead>";
            echo "</tr>";
    ...
    public function createPagination($controller) {
        if ($this->model) {
            echo "<div arial-label='Page navigation example'>";
            echo "<ul class='pagination'>";
            echo "<li class='page-item'><a class='page-link' href=\"Request.php?class=$controller&method=showList&page={$this->previousPage}\">Anterior</a></li>";
            echo "<li class='page-item'><strong class='page-link'>Página {$this->currentPage} de {$this->lastPage}</strong></li>";
            echo "<li class='page-item'><a class='page-link' href=\"Request.php?class=$controller&method=showList&page={$this->nextPage}\">Próximo</a></li>";
            echo "<ul>";
            echo "<div>";
        }
    }
